...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        hold_period = sys.argv[1]  # 从外部传入hold_period参数
        offset = int(sys.argv[2])  # 从外部传入offset参数
        # 判断外部是否传入了factor_list参数
        if len(sys.argv) > 3:
            factor_info = sys.argv[3]  # 从外部传入factor_list参数
            factors = str_to_factor_info(factor_info)
            factor_list = []
            for k, v in factors.items():
                factor, param = k.split('_')
                factor_list.append((factor, v, eval(param), 1))
    ...
    long_df, short_df = Strategy.before_filter(back_test_df)
    ...
    if if_manual:
        ...
        # 判断外部是否传入了factor_list参数
        if len(sys.argv) > 3:  # 传入了factor_list参数，表示'4_遍历选币参数.py'运行，需要保存一下遍历的数据
            # 保存数据到指定文件
            rtn_df = rtn.T
            rtn_df['offset'] = offset
            rtn_df['持仓周期'] = hold_period
            rtn_df['因子数量'] = len(factor_list)
            rtn_df['因子组合'] = str(sorted(factors.items(), key=lambda x: x[0]))
            rtn_df = rtn_df[['offset', '持仓周期', '因子组合', '累积净值', '最大回撤', '胜率', '盈亏收益比', '最大连续盈利周期数', '最大连续亏损周期数',
                             '最大回撤开始时间', '最大回撤结束时间', '年化收益/回撤比', '每周期平均换手率', '收益率标准差', '因子数量']]
            save_file_path = root_path + f'/data/rtn_{strategy_name}_{hold_period}.csv'
            if os.path.exists(save_file_path):
                rtn_df.to_csv(save_file_path, encoding='gbk', index=False, header=False, mode='a')
            else:
                rtn_df.to_csv(save_file_path, encoding='gbk', index=False)
