...
# 多头的选币因子列名。
long_factor = '因子1'  # 因子：表示使用复合因子，默认是 factor_list 里面的因子组合。需要修改 calc_factor 函数配合使用
...
factor_list = [
    ...
    ('TradeNumMeanV1034HH8', True, 168, 1)
]
...
def calc_factor(df, **kwargs):
    ...
    df[long_factor] = df[str(_factor_list[0][0]) + '_' + str(_factor_list[0][2]) + '_rank']
    df[short_factor] = df[str(_factor_list[1][0]) + '_' + str(_factor_list[1][2]) + '_rank']
...
def before_filter(df, **kwargs):
    ...
    # 配置了过滤因子信息，则进行过滤操作
    df_long['filter_rank1'] = df_long.groupby('candle_begin_time')['PctChange_168'].rank(ascending=True, pct=True)
    df_long = df_long[(df_long['filter_rank1'] < 0.8)]
    df_short['filter_rank22'] = df_short.groupby('candle_begin_time')['CciHH8NoS_168'].rank(ascending=True, pct=True)
    df_short = df_short[(df_short['filter_rank22'] < 0.6)]
